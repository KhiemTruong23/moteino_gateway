...
#if REGISTER_DETAIL
// SERIAL PRINT
// replace SerialX.print("string") with SerialPrint("string")
...
void ForkedRFM69::readAllRegs()
{
  ...
  SerialX.println("Address - HEX - BIN");
...
void ForkedRFM69::readAllRegsCompact() {
  ...
  for ( uint8_t reg = 0x01; reg<0x80; reg++ ) {
    ...
    if ( ret < 0x10 ) SerialX.print("0");  // Handle values less than 10
    SerialX.print( ret, HEX);
    SerialX.print(" ");
  }
}
...
#ifdef RF69_WL_DEBUG
  SerialX.print("Sending burst for ");
  SerialX.print(cycleDurationMs, DEC);
  SerialX.println(" ms");
#endif
