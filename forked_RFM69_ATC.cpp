...
#include "forked_RFM69.h"   // include the RFM69 library files as well
#include "RFM69registers.h"
#include <SPI.h>

volatile uint8_t ForkedRFM69_ATC::ACK_RSSI_REQUESTED;  // new type of flag on ACK_REQUEST

//=============================================================================
// initialize() - some extra initialization before calling base class
//=============================================================================
bool ForkedRFM69_ATC::initialize(uint8_t freqBand, uint16_t nodeID, uint8_t networkID) {
  ...
  return ForkedRFM69::initialize(freqBand, nodeID, networkID);  // use base class to initialize most everything
}
...
void ForkedRFM69_ATC::sendACK(const void* buffer, uint8_t bufferSize) {
...
void ForkedRFM69_ATC::sendFrame(uint16_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK) {
...
//=============================================================================
void ForkedRFM69_ATC::sendFrame(uint16_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK, bool sendRSSI, int16_t lastRSSI) {
...
//=============================================================================
void ForkedRFM69_ATC::interruptHook(uint8_t CTLbyte) {
...
//=============================================================================
bool ForkedRFM69_ATC::sendWithRetry(uint16_t toAddress, const void* buffer, uint8_t bufferSize, uint8_t retries, uint8_t retryWaitTime) {
  uint32_t sentTime;
...
//=============================================================================
void ForkedRFM69_ATC::receiveBegin() {
  ACK_RSSI_REQUESTED = 0;
  ForkedRFM69::receiveBegin();
}
...
void  ForkedRFM69_ATC::enableAutoPower(int16_t targetRSSI){    // TomWS1: New method to enable/disable auto Power control
...
//=============================================================================
int16_t  ForkedRFM69_ATC::getAckRSSI(void){                     // TomWS1: New method to retrieve the ack'd RSSI (if any)
